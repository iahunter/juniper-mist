...
|---|---|---:|---|---|
|`full_duplex`|query|false|`boolean`|Indicates full or half duplex|
|`disabled`|query|false|`boolean`|Indicates if interface is disabled|
|`mac`|query|false|`string`|Device identifier|
|`device_type`|query|false|`#/components/schemas/search_site_sw_or_gw_ports_device_type`|Device type|
|`neighbor_mac`|query|false|`string`|Chassis identifier of the chassis type listed|
|`neighbor_port_desc`|query|false|`string`|Description supplied by the system on the interface E.g. "GigabitEthernet2/0/39"|
|`neighbor_system_name`|query|false|`string`|Name supplied by the system on the interface E.g. neighbor system name E.g. "Kumar-Acc-SW.mist.local"|
|`poe_disabled`|query|false|`boolean`|Is the POE configured not be disabled.|
...
|`up`|query|false|`boolean`|Indicates if interface is up|
|`active`|query|false|`boolean`|Indicates if interface is active/inactive|
|`jitter`|query|false|`number`|Last sampled jitter of the interface|
|`loss`|query|false|`number`|Last sampled loss of the interface|
|`latency`|query|false|`number`|Last sampled latency of the interface|
|`stp_state`|query|false|`#/components/schemas/search_site_sw_or_gw_ports_stp_state`|If `up`==`true`|
|`stp_role`|query|false|`#/components/schemas/search_site_sw_or_gw_ports_stp_role`|If `up`==`true`|
|`xcvr_part_number`|query|false|`string`|Optic Slot Partnumber, Check for null/empty|
|`auth_state`|query|false|`#/components/schemas/search_site_sw_or_gw_ports_auth_state`|If `up`==`true` && has Authenticator role|
|`lte_imsi`|query|false|`string`|LTE IMSI value, Check for null/empty|
|`lte_iccid`|query|false|`string`|LTE ICCID value, Check for null/empty|
|`lte_imei`|query|false|`string`|LTE IMEI value, Check for null/empty|
