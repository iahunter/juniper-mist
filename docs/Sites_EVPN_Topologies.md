...
$resp = $client->sites evpn topologies->createSiteEvpnTopology($model->toArray());
print_r($resp);
```
...
$resp = $client->sites evpn topologies->updateSiteEvpnTopology($model->toArray());
print_r($resp);
```
