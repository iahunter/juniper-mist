...
$resp = $client->sites asset filters->createSiteAssetFilter($model->toArray());
print_r($resp);
```
...
$resp = $client->sites asset filters->updateSiteAssetFilter($model->toArray());
print_r($resp);
```
